...
printf("      LINE: %d, nrows == %d\n", __LINE__, nrows);
	/* Allocate the table structure from general memory */
	h = (SFXHASH*)malloc(sizeof(SFXHASH));
	if (!h)
	{
		return 0;
	}
...
printf("      LINE: %d, h->mc.memused == %u \n", __LINE__, h->mc.memused);
printf("      LINE: %d, h->mc.nblocks == %d \n", __LINE__, h->mc.nblocks);
printf("      LINE: %d, h->mc.memcap == %u \n", __LINE__, h->mc.memcap);
...
static SFXHASH_NODE * sfxhash_find_node_row( SFXHASH * t, void * key, int * rindex )
{
    unsigned       hashkey;
    int            index;
    SFXHASH_NODE  *hnode;
